        ...
        <div className="flex justify-between h-16 items-center">
          <div className="flex-shrink-0">
            <Link href="/" className="text-2xl font-bold tracking-tight">
              Botanik<span className="text-indigo-200">Shop</span>
            </Link>
          </div>
          ...
          <form onSubmit={submitHandler} className="hidden lg:flex flex-1 max-w-md mx-8">
            ...
            />
            <button type="submit" className="bg-indigo-500 px-4 py-1 flex items-center justify-center rounded-r-md hover:bg-indigo-400">
          ...
          <nav className="hidden md:flex space-x-8 items-center">
            <Link href="/cart" className="flex items-center hover:text-indigo-200 transition">
            ...
            {userInfo ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center hover:text-indigo-200 transition focus:outline-none"
                >
                  <svg className="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Link href="/login" className="flex items-center hover:text-indigo-200 transition">
                <svg className="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                Sign In
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-indigo-700">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <Link href="/cart" className="block px-3 py-2 rounded-md hover:bg-indigo-600">
              Cart {cartCount > 0 && `(${cartCount})`}
            </Link>
            {userInfo ? (
              <>
                <Link href="/profile" className="block px-3 py-2 rounded-md hover:bg-indigo-600">Profile</Link>
                {userInfo.isAdmin && (
                  <>
                    <Link href="/admin/userlist" className="block px-3 py-2 rounded-md hover:bg-indigo-600 text-indigo-300">Admin: Users</Link>
                    <Link href="/admin/productlist" className="block px-3 py-2 rounded-md hover:bg-indigo-600 text-indigo-300">Admin: Products</Link>
                    <Link href="/admin/orderlist" className="block px-3 py-2 rounded-md hover:bg-indigo-600 text-indigo-300">Admin: Orders</Link>
                  </>
                )}
                <button onClick={logout} className="block w-full text-left px-3 py-2 rounded-md hover:bg-indigo-600">
                  Logout
                </button>
              </>
            ) : (
              <Link href="/login" className="block px-3 py-2 rounded-md hover:bg-indigo-600">Sign In</Link>
            )}
          </div>
        </div>
      )}
